...
class TestBuiltinOperator:
    ...
    @allure.title("注册内置函数算子，传参正确，注册成功")
    def test_builtin_operator_function_01(self, Headers):
        op_id = str(uuid.uuid4())
        name = "builtin_func_" + ''.join(random.choice(string.ascii_letters) for i in range(8))
        
        # 内部 function_input 结构
        function_input = {
            "name": name,
            "description": "builtin function desc",
        ...
        result = self.client.RegisterBuiltinOperator(data, Headers)
        assert result[0] == 200
        assert result[1]["operator_id"] == op_id
